...
func change_to_dungeon_map():
    ...
    var prep_manager = get_node_or_null("PreparationManager") # Node within PreparationScene
    if prep_manager:
        ...
        print_rich("[color=yellow]GameManager Warning:[/color] PreparationManager not found at path 'PreparationManager' during _ready(). Signals may not connect.")

    var map_manager = get_node_or_null("DungeonMapManager")
    if map_manager:
        ...
        print_rich("[color=yellow]GameManager Warning:[/color] DungeonMapManager not found at path 'DungeonMapManager' during _ready().")
    ...
    if combat_manager:
        ...
        print_rich("[color=yellow]GameManager Warning:[/color] CombatManager not found at path 'CombatManager' during _ready().")

    var post_battle_manager = get_node_or_null("PostBattleManager")
    if post_battle_manager:
        ...
        print_rich("[color=yellow]GameManager Warning:[/color] PostBattleManager not found at path 'PostBattleManager' during _ready().")

    var rest_manager = get_node_or_null("RestManager")
    if rest_manager:
        ...
        print_rich("[color=yellow]GameManager Warning:[/color] RestManager not found at path 'RestManager' during _ready().")
...
func _notify_preparation_manager_to_load_data():
    var prep_manager = get_node_or_null("PreparationManager") # Adjust path if it's inside PrepScene
