...
func generate_procedural_map() -> void:
	...
	for i in range(node_count):
		...
		var node_data := {"id": i, "type": node_type, "connections": [], "specific_data": {}}  # IDs of nodes this node connects to  # Holds data specific to the node type
...
func on_node_button_pressed(node_id: int) -> void:
	...
	if selected_node_data:
		...
		if (
			current_node_data
			and node_id in current_node_data.connections
			and node_id not in visited_node_ids
		):
...
func handle_node_interaction(_node_data: Dictionary) -> void:
	...
	match node_data.type:
		...
		"loot":
			...
			emit_signal(
				"transition_to_loot_event", {"type": "loot", "data": node_data.specific_data}
			)
		"event", "trap":  # Traps are handled as events
			...
			emit_signal(
				"transition_to_loot_event", {"type": "event", "data": node_data.specific_data}
			)
		"rest":
	...
	var events = [
		{
			"event_id": "shrine_01",
			"description":
			"A mysterious shrine radiates a faint energy. Do you attempt to decipher the ancient runes?",
			"options":
			[
				{
					"text": "Decipher Runes (Perception DC 12)",
					"action": "skill_check",
					"skill": "perception",
					"dc": 12
				},
			...
			"outcomes":
			{
				"skill_check_success":
				{
					"text": "You gain a burst of insight and feel slightly rejuvenated!",
					"effects": {"heal_party_small": true}
				},
				"skill_check_failure":
				{
					"text": "The runes shift menacingly, and a wave of fatigue washes over you.",
					"effects": {"fatigue_party_small": true}
				},
	...
	var traps = [
		{
			...
			[
				{
					"text": "Attempt to dodge (Agility DC 10)",
					"action": "skill_check",
					"skill": "agility",
					"dc": 10
				}
			...
			"outcomes":
			{
				...
				"skill_check_failure":
				{
					"text": "The spikes catch you off guard!",
					"effects": {"damage_party_small": true, "amount": 5}
				}
			}
		},
		{
			...
			[
				{
					"text": "Try to block or parry (Defense DC 11)",
					"action": "skill_check",
					"skill": "defense",
					"dc": 11
				}
			...
			"outcomes":
			{
				...
				"skill_check_failure":
				{
					"text": "A few darts find their mark, searing with poison!",
					"effects": {"damage_party_small": true, "amount": 3, "status": "poisoned"}
				}
			}
		}
	]
...
func _on_event_panel_resolved(
	resolved_outcome_data: Dictionary, _original_event_data: Dictionary
) -> void:
		...
		if effects.has("damage_party_small"):
			...
			for i in range(1, 6):  # Assuming 5 members max
				...
				if current_party_status.has(key):
					current_party_status[key].hp = max(
						current_party_status[key].hp - damage_amount, 0
					)
