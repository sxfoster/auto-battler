...
func update_party_status_display():
	...
	for i in range(member_nodes.size()):
		...
		if stat_label:
			...
			print(
				"Error: Label node not found in PartyStatusGrid child: ", member_node_container.name
			)
...
func _on_use_food_drink_pressed(item_effect_data: Dictionary):
	...
	if (
		party_members_data[target_member_key].has(stat_to_change)
		and party_members_data[target_member_key].has(max_stat_key)
	):
		...
		add_rest_log_entry(
			(
				"Applied %s to %s. New %s: %d"
				% [
					item_effect_data.get("name", "Item"),
					party_members_data[target_member_key].name,
					stat_to_change,
					party_members_data[target_member_key][stat_to_change]
				]
			)
		)
	else:
		add_rest_log_entry(
			(
				"Error: Stat '%s' or '%s' not found for %s."
				% [stat_to_change, max_stat_key, party_members_data[target_member_key].name]
			)
		)
...
func _on_continue_button_pressed():
	...
	for member_key in party_members_data:
		if (
			party_members_data[member_key].has("fatigue")
			and party_members_data[member_key].has("max_fatigue")
		):
			party_members_data[member_key].fatigue = max(
				0, party_members_data[member_key].fatigue - 10
			)  # Recover 10 fatigue
