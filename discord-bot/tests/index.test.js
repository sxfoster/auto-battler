jest.mock('node:fs', () => ({ existsSync: jest.fn(() => false), readdirSync: jest.fn(() => []) }));
jest.mock('fs', () => ({ existsSync: jest.fn(() => false), readdirSync: jest.fn(() => []) }));

jest.mock('../util/config', () => ({ DISCORD_TOKEN: 'token' }));

jest.mock('../src/utils/interactionRouter', () => ({
  routeInteraction: jest.fn()
}));
...
const router = require('../src/utils/interactionRouter');

require('../index.js');

test('interaction routed through router', async () => {
  ...
  expect(router.routeInteraction).toHaveBeenCalledWith(interaction);
});
