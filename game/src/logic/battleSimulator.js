...
export function simulateBattle(partyData, enemyData) {
  ...
  while (aliveParties && aliveEnemies) {
    ...
    for (const unit of turnOrder) {
      ...
      const opposingTeamMembers = parties.includes(unit) ? enemies : parties;
      const livingOpponents = opposingTeamMembers.filter(u => u.currentHp > 0);
      ...
      if (playedCard.targetType === 'any') {
          validTargets = [...livingOpponents]; // This card can target anyone alive on the opposing team.
      } else {
          // Default behavior: use the positioning rules against living opponents.
          validTargets = livingOpponents.filter(enemy => isTargetable(enemy, livingOpponents));
      }

      // If there are no valid targets, the unit cannot act.
      if (validTargets.length === 0) {
        // Log no target
        addStep(unit.id, 'noTarget', { cardId: playedCard.id }, [], prePlay, snapshot(parties, enemies), `${unit.id} has no valid targets for ${playedCard.id}.`);
      ...
      // --- END REFACTOR ---
      if (!target) continue; // Should not happen if validTargets.length > 0
...
// Assign battle decks
// Note: The MOCK_HEROES/ENEMIES don't have 'battleDeck' by default in the new structure.
// We need to ensure the normalizeUnit function or direct data manipulation handles this.
// For simplicity, let's assume partyData/enemyData for simulateBattle will include 'deck' or 'cards'.
// The testPlayerParty and testEnemyParty are structured like the old UnitState,
// so they should be compatible with normalizeUnit if it correctly maps fields.
// Let's ensure they have a 'deck' property as normalizeUnit expects.

testPlayerParty[0].deck = [MOCK_CARDS.ARROW_SHOT, MOCK_CARDS.EAGLE_EYE];
testPlayerParty[1].deck = [MOCK_CARDS.INSPIRE, MOCK_CARDS.LULLABY];
// Enemies will use their default deck from MOCK_ENEMIES if not overridden here.

// The simulateBattle function expects partyData and enemyData.
// These should be arrays of objects that normalizeUnit can process.
// MOCK_HEROES.RANGER and MOCK_ENEMIES.GOBLIN are objects, so [MOCK_HEROES.RANGER] is an array of one unit.

const finalLog = simulateBattle(testPlayerParty, testEnemyParty);
console.log("--- BATTLE LOG ---");
finalLog.forEach(step => {
    console.log(`[${step.actionType}] ${step.logMessage}`); // Adjusted to new log format (actionType, logMessage)
});
// Manually inspect the log to ensure "Goblin Back" is not targeted until "Goblin Front" is defeated
// by non-'any' targetType attacks, and ARROW_SHOT can target "Goblin Back".
