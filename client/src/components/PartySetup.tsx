...
const PartySetup: React.FC = () => {
  ...
  const setAvailableClasses = useGameStore(
    (state) => state.setAvailableClasses,
  );
  const load = useGameStore((state) => state.load);

  // reload from storage whenever this component mounts
  useEffect(() => {
    console.log("PartySetup mounted");
  ...
  useEffect(() => {
    if (availableClasses.length === 0 && selectedCharacters.length < 5) {
      const remaining = allClasses.filter(
        (c) => !selectedCharacters.find((pc) => pc.class === c.id),
      );
      setAvailableClasses(
        getRandomClasses(Math.min(4, remaining.length), remaining),
      );
    }
  ...
  const rerollAvailableClasses = (current: PartyCharacter[]) => {
    const remaining = allClasses.filter(
      (cls) => !current.find((pc) => pc.class === cls.id),
    );
    if (current.length >= 5) {
      setAvailableClasses([]);
    } else {
  ...
  const handleCardAssign = (characterId: string, card: Card) => {
    setSelectedCharacters(
      selectedCharacters.map((pc) => {
        if (
          pc.id === characterId &&
          pc.assignedCards.length < 2 &&
          !pc.assignedCards.find((c) => c.id === card.id)
        ) {
    ...
    setSelectedCharacters(
      selectedCharacters.map((pc) => {
        if (pc.id === characterId) {
          return {
            ...pc,
            assignedCards: pc.assignedCards.filter((c) => c.id !== cardId),
          };
        }
        return pc;
      }),
          ...
          {availableClasses.length > 0 && (
            <>
              <button
                ...
                title={
                  rerollCount >= 2
                    ? "Maximum rerolls reached"
                    : "Try a new set of classes for your adventure!"
                }
              >
                Reroll Classes
              </button>
              <span className={styles.actionCounter}>
                Rerolls left: {2 - rerollCount}
              </span>
            </>
          )}
        </div>
        ...
        {selectedCharacters.length > 0 && (
          <div style={{ textAlign: "center", marginBottom: "1rem" }}>
            <button
              ...
              title={
                undoCount >= 2
                  ? "Maximum undos reached"
                  : "Remove the most recently added class"
              }
            >
              Undo Last Pick
            </button>
            <span className={styles.actionCounter}>
              Undos left: {2 - undoCount}
            </span>
          </div>
        )}

        {/* Selected Characters and Card Assignment Section */}
        <div className={styles.selectedCharactersArea}>
          {" "}
          {/* Apply .selectedCharactersArea */}
          <h2 className={styles.sectionTitle}>Configure Your Party</h2>
          {selectedCharacters.length === 0 && (
            <p className={styles.infoText}>
              No classes selected yet. Click on a class above to add it to your
              party.
            </p>
          )}
          {selectedCharacters.map((pc) => {
            ...
            return (
              <div key={pc.id} className={styles.selectedCharacterPanel}>
                {" "}
                {/* Apply .selectedCharacterPanel */}
                <div className={styles.characterPanelHeader}>
                  {" "}
                  {/* Apply .characterPanelHeader */}
                  <h3>
                    {pc.name} (Class: {clsDef ? clsDef.name : "Unknown"})
                  </h3>
                  <button
